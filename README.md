...
| 변수 | 용도 | 필수 여부 | 비고 |
| --- | --- | --- | --- |
| `OPENAI_API_KEY` | `/api/analyze` OpenAI Chat Completions 키 | 선택 (미설정 시 오류 응답) | Cloudflare Pages Secret 권장 |
...
## 테스트/검증 로그
- 2025-10-04 `npm run build` (성공: Google 로그인 자동 재시도 로직, 커뮤니티 헤더 링크, 관리자 레이트 리밋 포함)
...
_Last updated: 2025-10-04 (Google 로그인 자동 재시도 UX · 관리자 레이트리밋 · 커뮤니티 헤더 링크)_
