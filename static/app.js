...
function resolvePageHref(page) {
  const normalizedPage = typeof page === 'string' ? page.replace(/^\.\/?/, '') : ''
  const base = getBasePath()
  if (!normalizedPage) {
  ...
  return `${normalizedBase}${normalizedPage}`
}

function resolvePlansLinkHref() {
  return resolvePageHref('plans.html')
}
...
async function init() {
  ...
  if (initialView === 'admin') {
    ...
    if (hasCommunityAccess()) {
      ...
      runtime.lastAllowedView = 'home'
      setView('home', { force: true, bypassAccess: true })
      const homeRoute = joinBasePath(VIEW_ROUTES.home)
      if (window.location.pathname !== homeRoute) {
        window.history.replaceState({ view: 'home' }, '', homeRoute)
      }
    }
  } else {
    setView('home', { force: true, bypassAccess: true })
